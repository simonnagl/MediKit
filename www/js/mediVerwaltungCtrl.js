	...
	$scope.validateMedi = function(medi) {
		for(prop in medi) {
			switch(prop) {
				...
				case "snoozemin":
					if(medi[prop] == undefined || medi[prop] == "") {
						$cordovaDialogs.alert('Snoozezeit ist leer.', 'Bitte eintragen.', 'OK')
							.then(function() {
							});
						return false;
					};
					if(medi[prop] < '0') {
						$cordovaDialogs.alert('Snoozezeit ist negativ.', 'Bitte korrigieren.', 'OK')
							.then(function() {
							});
						return false;
					};
			...
			switch(prop) {
				case "menge":
					if(groesse[prop] == undefined || groesse[prop] == "") {
						$cordovaDialogs.alert('Packungsgröße ist leer.', 'Bitte eintragen.', 'OK')
							.then(function() {
							});
						return false;
					};
